canvas = document.getElementById("myCanvas");
ctx = canvas.getContext("2d");

greencar_height = 100;
greencar_width = 75;

background_image = "parkingLot.jpg";
greencar_image = "car2.png";

greencar_x = 5;
greencar_y = 225;

function add() {
	background_imaTag = new Image();
	background_imaTag.onload = uploadBackground;
	background_imaTag.src = background_image;

	greencar_imaTag = new Image();
	greencar_imaTag.onload = uploadgreencar;
	greencar_imaTag.src = greenacar_image;
}


function uploadBackground() {
 ctx.drawImage(background_imaTag, 0, 0, canvas.width, canvas.height);
}

function uploadgreencar() {
	ctx.drawImage(background_imaTag, greencar_y, greencar_x, greenacr.width, greencar.height);
}


window.addEventListener("keydown", my_keydown);

function my_keydown(e)
{
	keyPressed = e.keyCode;
	console.log(keyPressed);
		if(keyPressed == '38')
		{
			up();
			console.log("up");
		}
	
		if(keyPressed == '40')
		{
			down();
			console.log("down");
		}
		
		if(keyPressed == '37')
		{
			left();
			console.log("left");
		}
	
		if(keyPressed == '39')
		{
			right();
			console.log("right");
		}
		
		
}

function up()
{
	if(greencar_y >=0)
	{
		greencar_y=greencar_y - 10;
		console.log("when up arrow key is pressed, x = " + greencar_x + " y = " + greencar_y );
		uploadBackground();
		uploadgreencar();
	}
}

function down()
{
	if(greencar_x <=400)
	{
		greencar_x=greencar_x + 10;
		console.log("when down arrow key is pressed, x = " + greencar_x + " x = " + greencar_y );
		uploadBackground();
		uploadgreencar();
	}
}

function left()
{
	if(greencar_y >=0)
	{
		greencar_y=greencar_y - 10;
		console.log("when left arrow key is pressed, x = " + greencar_x + " y = " + greencar_y );
		uploadBackground();
		uploadgreencar();
	}
}

function right()
{
	if(greencar_y <=400)
	{
		greencar_x=greencar_x + 10;
		console.log("when right arrow key is pressed, x = " + greencar_x + " y = " + greencar_y );
		uploadBackground();
		uploadgreencar();
	}
}
